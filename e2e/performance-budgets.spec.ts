...
test.describe("Performance Budgets", () => {
	...
	test("should have minimal memory usage", async ({ page, context }) => {
		await page.goto("/");
	...
	test("should have acceptable Time to Interactive", async ({ page }) => {
		...
		console.log(`   DOM Interactive: ${Math.round((metrics as any).domInteractive)}ms`);
		console.log(`   DOM Content Loaded: ${Math.round((metrics as any).domContentLoaded)}ms`);
		console.log(`   Load Complete: ${Math.round((metrics as any).loadComplete)}ms`);
		
		// Budget: DOM Interactive < 2000ms
		expect((metrics as any).domInteractive).toBeLessThan(2000);
	});
});
