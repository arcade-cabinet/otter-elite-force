...
const waitForStable = async (page: any, ms = 2000) => {
...
test.describe("Visual Audit - Screenshot Generation", () => {
  test("capture key game states for visual analysis", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '03-cutscene-start.png'), fullPage: true });
    ...
    if (await nextBtn.isVisible()) {
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '04-cutscene-line-2.png'), fullPage: true });
    ...
    if (await canvas.isVisible()) {
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '05-gameplay-start.png'), fullPage: true });
        ...
        if (await actionCluster.isVisible()) {
            await actionCluster.screenshot({ path: path.join(SCREENSHOT_DIR, '07-action-buttons.png') });
    ...
    await waitForStable(page, 3000);
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '08-canteen-platoon.png'), fullPage: true });
    console.log("Captured Canteen Platoon");

    // 8. Canteen Upgrades
    await page.locator('button:has-text("UPGRADES")').click();
    await waitForStable(page, 1000);
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, '09-canteen-upgrades.png'), fullPage: true });
