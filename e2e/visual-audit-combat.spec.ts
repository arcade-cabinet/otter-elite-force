  ...
  test("capture combat and objective states", async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        const state = {
            ...
            strategicObjectives: { siphonsDismantled: 0, villagesLiberated: 0, gasStockpilesCaptured: 0, healersProtected: 0, alliesRescued: 0 },
            spoilsOfWar: { creditsEarned: 0, clamsHarvested: 0, upgradesUnlocked: 0 },
            upgrades: { speedBoost: 0, healthBoost: 0, damageBoost: 0, weaponLvl: { "service-pistol": 1 } },
            isLZSecured: true,
            baseComponents: [],
    ...
    if (await canvas.isVisible()) {
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '01-combat-scene.png'), fullPage: true });
        ...
        await page.screenshot({ path: path.join(SCREENSHOT_DIR, '03-fall-warning-overlay.png'), fullPage: true });
