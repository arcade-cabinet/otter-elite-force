	...
	(
		...
		useFrame((state) => {
			...
			if (isClimbing) {
				...
				if (armLRef.current) armLRef.current.rotation.x = Math.PI / 4 + Math.sin(time * 10 + Math.PI) * 0.2;
		...
		return (
			<group ref={ref} position={position} rotation-y={rotation}>
				...
				<group ref={headRef} position={[0, 1.45, 0]}>
					...
					{[-1, 1].map((side) => (
						<mesh key={`ear-${side}`} position={[side * 0.38, 0.38, 0]} rotation-z={side * 0.5} material={matFur}>
