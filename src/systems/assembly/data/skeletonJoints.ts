/**
 * Skeletal Rig Definitions
 *
 * Universal character skeleton used by Player, Enemies, Villagers.
 * Same rig for all characters - just different meshes attached.
 */
...
export const UNIVERSAL_SKELETON: JointDef[] = [
	...
	},
	{
		...
		rotationLimits: { x: { min: 0, max: 0 }, y: { min: 0, max: 2.5 }, z: { min: 0, max: 0 } },
	},
	{
		name: "FOREARM_L",
		parent: "ELBOW_L",
 ...
 * Get joint by name
 */
export function getJoint(name: JointName): JointDef | undefined {
	return UNIVERSAL_SKELETON.find((joint) => joint.name === name);
}

/**
 * Get all children of a joint
 */
export function getJointChildren(name: JointName): JointDef[] {
	return UNIVERSAL_SKELETON.filter((joint) => joint.parent === name);
}

/**
 * Get the joint chain from root to target
 */
export function getJointChain(targetName: JointName): JointDef[] {
	const chain: JointDef[] = [];
	let current = getJoint(targetName);
