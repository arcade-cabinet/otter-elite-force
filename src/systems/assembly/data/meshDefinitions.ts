/**
 * Mesh Definitions Library
 *
 * All reusable mesh geometries defined ONCE and referenced by ID.
 * Materials are applied separately based on faction.
 */
...
/**
 * Geometry type for mesh creation
 */
export type GeometryType = "BOX" | "CYLINDER" | "SPHERE" | "CAPSULE" | "CONE" | "TORUS" | "CUSTOM";
...
export interface MeshDef {
	...
	/** Parameters for THREE geometry constructor */
...
/**
 * Get mesh definition by ID
 */
export function getMeshDef(id: MeshId): MeshDef {
...
export function getAllMeshIds(): MeshId[] {
	return Object.keys(MESH_LIBRARY) as MeshId[];
}

/**
 * Get mesh IDs by category
 */
export function getMeshIdsByCategory(
	category: "structural" | "weapon" | "character" | "equipment",
): MeshId[] {
	const categoryPrefixes: Record<string, string[]> = {
		structural: ["STILT", "FLOOR", "WALL", "ROOF", "LADDER", "RAILING", "ROPE"],
		weapon: ["BARREL", "RECEIVER", "STOCK", "GRIP", "MAGAZINE", "SCOPE"],
		character: ["TORSO", "HEAD", "ARM", "LEG", "HAND", "FOOT", "TAIL"],
		equipment: ["VEST", "HELMET", "BACKPACK"],
	};

	const prefixes = categoryPrefixes[category] || [];
	return getAllMeshIds().filter((id) => prefixes.some((prefix) => id.startsWith(prefix)));
}
