...
export function GameCanvas({ deploymentData }: GameCanvasProps) {
	...
	useEffect(() => {
		const onStartBuild = (data: { buildingType: string }) => {
			...
			if (
				(cost.fish ?? 0) > res.fish ||
				(cost.timber ?? 0) > res.timber ||
				(cost.salvage ?? 0) > res.salvage
			) {
	...
	const terrainCanvas = useMemo(() => (mission ? paintMinimapTerrain(mission) : null), [mission]);

	// ─── Scenario Engine ───

	const scenarioRef = useRef<{ engine: ScenarioEngine; worldQuery: ScenarioWorldQuery } | null>(
		null,
	);
				...
				if (unitDef) {
					for (let i = 0; i < (action.count ?? 1); i++) {
						spawnUnit(
							world,
							unitDef,
							action.position.x + (Math.random() - 0.5) * 2,
							action.position.y + (Math.random() - 0.5) * 2,
							action.faction,
						);
					}
				}
		...
		>
			<Stage width={size.width} height={size.height}>
				...
				<FogLayer
					...
					viewportW={size.width}
					viewportH={size.height}
					worldTilesW={Math.ceil(worldW / 32)}
					worldTilesH={Math.ceil(worldH / 32)}
				/>
			</Stage>
