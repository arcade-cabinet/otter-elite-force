...
function drawOtterBase(ctx: Ctx): void {
	...
	p(ctx, 6, 3, PAL.black);
	p(ctx, 9, 3, PAL.black); // eyes
	p(ctx, 7, 4, PAL.otterNose);
	p(ctx, 8, 4, PAL.otterNose); // nose
...
function drawLodge(ctx: Ctx): void {
	...
	for (let i = 0; i < 40; i++)
		rect(ctx, 4 + _noise.next() * 22, 10 + _noise.next() * 18, 6, 2, PAL.otterBase);
...
function drawPredatorNest(ctx: Ctx): void {
	...
	p(ctx, 14, 16, PAL.gatorEye);
	p(ctx, 18, 16, PAL.gatorEye);
}
...
function drawFishSpot(ctx: Ctx): void {
	...
	rect(ctx, 5, 9, 2, 2, PAL.clamShell);
	p(ctx, 6, 9, PAL.stone);
	rect(ctx, 9, 11, 3, 2, PAL.clamShell);
	p(ctx, 10, 11, PAL.stone);
...
function drawMangroveTree(ctx: Ctx): void {
	...
	p(ctx, 7, 1, PAL.otterBase);
	p(ctx, 8, 1, PAL.otterBase);
	p(ctx, 8, 12, PAL.reedGreen);
	p(ctx, 9, 11, PAL.reedGreen);
}
...
const DRAW_FNS: Record<SpriteType, (ctx: Ctx) => void> = {
	...
	siphon_drone: (ctx) => {
		...
		for (let a = 0; a < 8; a++) {
			...
			p(
				ctx,
				Math.round(8 + Math.cos(angle) * 5),
				Math.round(6 + Math.sin(angle) * 5),
				PAL.waterShallow,
			);
		}
	},
	...
	barracks: (ctx) => {
		...
		for (let i = 0; i < 20; i++)
			p(ctx, 4 + _noise.next() * 24, 16 + _noise.next() * 12, PAL.mudDark); // wood grain
	...
	minefield: (ctx) => {
		...
		for (let i = 0; i < 15; i++)
			p(ctx, 4 + _noise.next() * 24, 20 + _noise.next() * 10, PAL.mudLight);
	...
	shield_generator: (ctx) => {
		...
		for (let a = 0; a < 12; a++) {
			...
			p(
				ctx,
				Math.round(16 + Math.cos(angle) * r),
				Math.round(14 + Math.sin(angle) * r),
				PAL.waterShallow,
			);
		}
		// Power conduits
