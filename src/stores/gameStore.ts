...
export const useGameStore = create<GameState>((set, get) => ({
	...
	discoverChunk: (x, z) => {
		...
		// Add Enemy Outposts (Structured clusters)
		if (rand() > 0.9 && id !== "0,0") {
			const outpostX = (rand() - 0.5) * 60;
			const outpostZ = (rand() - 0.5) * 60;
	...
	completeStrategic: (type) => {
		set((state) => ({
			saveData: {
				...
				strategicObjectives: {
					...state.saveData.strategicObjectives,
					gasStockpilesCaptured:
						type === "gas"
							? state.saveData.strategicObjectives.gasStockpilesCaptured + 1
							: state.saveData.strategicObjectives.gasStockpilesCaptured,
				},
			},
		}));
		get().saveGame();
		...
		set((state) => ({
			saveData: {
				...
				spoilsOfWar: {
					...
					creditsEarned:
						type === "credit"
							? state.saveData.spoilsOfWar.creditsEarned + 100
							: state.saveData.spoilsOfWar.creditsEarned,
					clamsHarvested:
						type === "clam"
							? state.saveData.spoilsOfWar.clamsHarvested + 1
							: state.saveData.spoilsOfWar.clamsHarvested,
				},
			},
		}));
		get().saveGame();
	...
	setLevel: (levelId) => {
		// Open world coordinate mapping for "levels"
		const levelCoords = [
			[0, 0],
			[10, 10],
			[-20, 20],
		...
		set({ playerPos: [x * CHUNK_SIZE, 0, z * CHUNK_SIZE] });
	},
	...
	buyUpgrade: (type, cost) => {
		if (get().spendCoins(cost)) {
			set((state) => {
				...
				const currentBoost = state.saveData.upgrades[boostKey];

				// Ensure we are only updating numbers
