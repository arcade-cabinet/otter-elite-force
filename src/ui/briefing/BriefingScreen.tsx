...
export function BriefingScreen({ briefing, onDeploy }: BriefingScreenProps) {
	...
	const missionLabel = useMemo(() => briefing.missionId.replace(/_/g, " ").toUpperCase(), [briefing.missionId]);
	...
	return (
		<BriefingShell
			...
			aside={<PortraitDisplay portraitId={briefing.portraitId} className="h-full min-h-[20rem] w-full sm:min-h-[26rem]" />}
			footer={
		...
		>
			<div className="grid gap-6">
				<ShellPanel title="Signal Traffic" description="Listen through the briefing feed before deployment.">
					<div
					...
					>
						{currentLine ? (
							<>
								...
								<p className="mt-3 min-h-28 font-body text-sm uppercase tracking-[0.12em] leading-relaxed text-foreground sm:text-base">
									{displayedText}
									{typedLength < fullText.length ? <span className="typewriter-cursor">▋</span> : null}
								</p>
									<div className="grid gap-2 rounded-md border border-accent/12 bg-accent/6 px-3 py-2">
									<div className="font-mono text-[10px] uppercase tracking-[0.22em] text-accent/80">Field Note</div>
										<p className="font-body text-[11px] uppercase tracking-[0.14em] leading-relaxed text-muted-foreground">
										Advance through the signal feed, confirm the threat picture, and make the final deploy click feel deliberate.
										</p>
									</div>
							</>
						) : null}
						...
						<div className="mt-4 flex gap-1">
							{briefing.lines.map((_, i) => (
								<div key={i} className={i <= lineIndex ? "h-1.5 w-8 bg-accent" : "h-1.5 w-8 bg-muted"} />
							))}
						</div>
					</div>
				</ShellPanel>
				...
				<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
					<ShellPanel title="Operational Notes" description="Mission tone, doctrine, and threat picture.">
						<div className="grid gap-3 md:grid-cols-3">
							<NoteBlock label="Theater" value="Copper-Silt Reach" body="Dense canopy, poor visibility, and shifting mud routes favor disciplined movement." />
									<NoteBlock label="Command" value="FIELD BRIEF" body="Use the briefing to set tempo, clarify the objective chain, and keep the next click readable." />
								<NoteBlock label="Priority" value="DEPLOY CLEAN" body="Proceed only after the briefing feed is clear and the objective chain is understood." />
						</div>
					</ShellPanel>
					...
					<ShellPanel title="Deploy Status" description="Final checks before insertion.">
							...
							<Badge variant="accent">TRANSMISSION {lineIndex + 1}/{briefing.lines.length}</Badge>
							<Badge>{briefing.portraitId.replace(/_/g, " ").toUpperCase()}</Badge>
								<div className="rounded-lg border border-border/70 bg-background/20 p-3">
									<div className="font-mono text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Drop Condition</div>
									<div className="mt-2 font-heading text-sm uppercase tracking-[0.18em] text-foreground">Green</div>
									<p className="mt-2 font-body text-[11px] uppercase tracking-[0.14em] leading-relaxed text-muted-foreground">Signal chain intact. Objective package loaded. Awaiting final command.</p>
								</div>
						</div>
					</ShellPanel>
				</div>
...
function NoteBlock({ label, value, body }: { label: string; value: string; body: string }) {
	return (
		<div className="rounded-lg border border-border/70 bg-background/25 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
			...
			<div className="mt-2 font-heading text-sm uppercase tracking-[0.18em] text-primary">{value}</div>
